...
public class CalculateBathsForWashingWelded {
    ...
    private Map<String, Double> calculateDetails(Map<String, Object> mapObject) {
        ...
        map.put("КФ1", norma);
        norma = 0 / 1000000 * 2 * 1.0;
        map.put("КФ2", norma);
        return map;
    }

    private Double calculateMaterials(Map<String, Object> mapObject, Map<String, Double> detailsMap) {
        Double sum = 0.0;
        Double norma = 0.0;
        Materials materials = materialsService.getByName("Лист 0,8 ст 430");
        norma = detailsMap.get("Стенка  передняя 0,8") + detailsMap.get("Скоба 0,8") + detailsMap.get("Стенка задняя 0,8");
        sum += norma * materials.getPrice();
        materials = materialsService.getByName("Лист 1,2 ст 430");
        if (mapObject.get("type").equals("нерж")) {
            norma = detailsMap.get("Стойка 1,2 390.002.002") + detailsMap.get("Уголок 1,2  390.002.101");
        } else {
            norma = (double) 0;
        }
        ...
        if (mapObject.get("type").equals("полимер")) {
            if (mapObject.get("type").equals("нерж")) {
                norma = detailsMap.get("Стойка 1,2 390.002.002") + detailsMap.get("Уголок 1,2  390.002.101") * 1.0;
            } else norma = detailsMap.get("Стойка 1,2 390.002.002") * 1.0;
        } else norma = (double) 0;
        sum += norma * materials.getPrice();
        ...
        if (mapObject.get("type").equals("полимер")) {
            if (mapObject.get("type").equals("нерж")) {
                norma = (double) 0;
            } else norma = detailsMap.get("Уголок 1,5 390.002.101") * 1.0;
        } else norma = (double) 0;
        sum += norma * materials.getPrice();
        ...
        if (mapObject.get("type").equals("полимер")) {
            norma = ((detailsMap.get("КФ1") + detailsMap.get("КФ2")) * 0.11);
        }
        sum += norma * materials.getPrice();
        return sum;
    }
